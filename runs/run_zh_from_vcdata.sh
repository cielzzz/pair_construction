#!/usr/bin/env sh
# ═══════════════════════════════════════════════════════════════
# 中文 + 只跑 edit：vcdata 已就绪 → editx → emotion → pair
# ═══════════════════════════════════════════════════════════════
# 用法：
#   sh run_zh_from_vcdata.sh                       ← 默认 RUN_MODE=submit（启智批量 editx）
#   RUN_MODE=after_editx sh run_zh_from_vcdata.sh  ← editx 已完成，遍历所有 split 跑本地 emotion+pair
#   RUN_MODE=local sh run_zh_from_vcdata.sh        ← 本地 demo（要求 LOCAL_SPLIT_NAME 存在）
#   RUN_MODE=submit DRY_RUN=1 sh run_zh_from_vcdata.sh
#
...
#   BATCH_ID 默认带 zh- 前缀（上游无前缀）            — 同目的
#   要恢复上游默认行为：export AUTO_RESUME_LATEST=1 export BATCH_ID=<自定>
#
# 改参数：编辑下方"参数区"
...
#   默认：对齐 run_zh_full.sh 的 stage1 输出目录（自己跑 vcdata 的产物）
#   备选：kxhuang 已跑好的 vc_mosstts 中文 vcdata 产物（跳过自己跑 stage1）
#         "/inspire/qb-ilm/project/embodied-multimodality/public/kxhuang/vc_mosstts/instruction_0.1_enzh/zh"
INPUT_VCDATA_ROOT="/inspire/qb-ilm2/project/embodied-multimodality/public/xyzhang/projects/vcdata_construction/outputs/instruction_0.1_enzh/zh"

# 仅 local 模式有效：单个 split 名（对应 INPUT_VCDATA_ROOT/<SPLIT_NAME>）
LOCAL_SPLIT_NAME="split_demo"

# 中文用 default.yaml
export PAIR_CONFIG="$(cd "$(dirname "$0")/.." && pwd)/configs/default.yaml"

# 把上游 vcdata 根传给下游 pair_construction（让 emotion/pair 阶段也用同一个根）
...
# ───── 执行 ───────────────────────────────────────────────────
echo "═══════════════════════════════════════════════"
echo " run_zh_from_vcdata  RUN_MODE=$RUN_MODE"
